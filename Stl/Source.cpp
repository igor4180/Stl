...
void main()
{
...
#ifdef DEQUE
	   // это контейнер, который хранит данные в виде списка векторов,
	  // это позволяет избежать переопределения памяти, и увеличить производительность
	...
	// В отличие от vector deque позволяет добавлять элементы не только в конец, но и в начало
